...
<template>
  <div class="container">
    <div class="textBox">
      <textarea placeholder="您的专业领域、擅长病种、研究方向等信息" v-model="majors"></textarea>
...
<script>
  ...
  export  default {
    ...
    data(){
      return {
        majors: this.$route.params.majors,
        fontNum:this.$route.params.majors.length,
        isActive:this.$route.params.majors ? true : false,
      }
    },
    created(){
    ...
    watch:{
      'majors' () {
        this.fontNum = this.majors.length;
        if(this.fontNum >400){
          this.majors = this.majors.substring(0, 400);
        }
    ...
    methods:{
      ...
      //Indicator.open('登录中');
      netWrokUtils.post('/wechat/' + gzhId + '/editMajors', params, (result) => {
